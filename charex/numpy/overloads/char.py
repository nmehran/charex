...
def ov_char_startswith(a, prefix, start=0, end=None):
    register_a, register_sub, a_dim, sub_dim = _get_register_types(a, prefix, 1)
    s, e = _ensure_slice(start, end)
    ...
    if a_dim > 0 or sub_dim > 0:
        def impl(a, prefix, start=0, end=None):
            ...
            return startswith(*register_a(a, False), *register_sub(prefix, False),
                              start, end)
    else:
        def impl(a, prefix, start=0, end=None):
            start = start or s
            end = e if end is None else end
            return np.array(startswith(*register_a(a, False), *register_sub(prefix, False),
                                       start, end)[0], 'bool')
    return impl
