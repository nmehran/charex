...
def _cast_comparison(len_chr, size_chr, len_cmp, size_cmp):
    ...
    cast_margin = size_chr - size_cmp
    if len_cmp == 1:
        size_stride = min(size_chr, size_cmp + (cast_margin < 0))
        size_cmp = 0
    else:
        size_stride = min(size_chr, size_cmp)
    return size_cmp, size_stride, cast_margin
...
def greater(chr_array, len_chr, size_chr, cmp_array, len_cmp, size_cmp, inv=False):
    ...
    size_cmp, size_stride, cast_margin = _cast_comparison(len_chr, size_chr, len_cmp, size_cmp)
    ...
    for i in range(len_chr):
        for j in range(size_stride):
            ...
            greater_than[i] = cast_margin > 0 and chr_array[stride + size_stride]
        stride += size_chr
        stride_cmp += size_cmp
    return greater_than
...
def greater_equal(chr_array, len_chr, size_chr, cmp_array, len_cmp, size_cmp, inv=False):
    ...
    for i in range(len_chr):
        for j in range(size_stride):
            ...
            greater_equal_than[i] = cast_margin >= 0 or not cmp_array[stride_cmp + size_stride]
        stride += size_chr
        stride_cmp += size_cmp
...
def less(chr_array, len_chr, size_chr, cmp_array, len_cmp, size_cmp, inv=False):
    ...
    for i in range(len_chr):
        for j in range(size_stride):
            ...
            less_than[i] = cast_margin < 0 and cmp_array[stride_cmp + size_stride]
        stride += size_chr
        stride_cmp += size_cmp
    return less_than
...
def less_equal(chr_array, len_chr, size_chr, cmp_array, len_cmp, size_cmp, inv=False):
    ...
    size_cmp, size_stride, cast_margin = _cast_comparison(len_chr, size_chr, len_cmp, size_cmp)
    less_equal_than = np.zeros(len_chr, 'bool')
    stride = stride_cmp = 0
    for i in range(len_chr):
        for j in range(size_stride):
            cmp_ord = chr_array[stride + j] - cmp_array[stride_cmp + j]
            cmp_ord = (inv and -cmp_ord) or cmp_ord
            if cmp_ord < 0:
                less_equal_than[i] = True
                break
            if cmp_ord > 0:
                break
        else:
            less_equal_than[i] = cast_margin <= 0 or not chr_array[stride + size_stride]
        stride += size_chr
        stride_cmp += size_cmp
